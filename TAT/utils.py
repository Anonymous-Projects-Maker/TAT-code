...
def expand_edge_index_and_timestamps(edge_index: List, timestamps: List, set_indice: List) -> (np.array, np.array):
    # test
...
class GraphDataset(Dataset):
    ...
    def show_statistics(self, name, logger):
        ...
        logger.info('{} set: {}, mean size: {:.1f}'.format(name, self.stat_array, self.mean_size, ))
...
def read_file(args, logger=None, pprocess=False, return_edgearray=False, normalize_time=False):
    ...
    if logger is not None:
        logger.info('Read in {}, # nodes: {}, # edges: {}'.format(
            dataset, G.number_of_nodes(), G.number_of_edges() ))
...
def sample_tri_wedge_neg_sets(G, logger, task=None, data_usage=1.0):
    ...
    logger.info('# triads: {}, # wedges: {}'.format(len(tri_samples), len(wedge_samples)))
...
def generate_set_indices(G, logger, args):
    """Generate set indeces, which are used for training/test target sets. But no labels now.
    # only triad prediction, 6 classes.
    """
    logger.info('Generating train/test set indices and labels from graph...')

    tri_samples, wedge_samples, neg_samples = sample_tri_wedge_neg_sets(G, logger, data_usage=args.data_usage)

    set_indices = tri_samples

    permutation = np.random.permutation(len(set_indices))
    set_indices = set_indices[permutation] # permute
    train_num = int(len(set_indices) * (1 - args.test_ratio) )
    ...
    logger.info('Generated {} train+test instances in total.'.format(len(set_indices)))
...
def extract_subgraph(G, set_indices, labels, prop_depth, layers, max_sp, parallel, cached_dir, debug, logger):
    logger.info("Extracting subgraphs and encoding...(parallel: {})".format(parallel))
    data_list = []
    ...
    if not parallel:
        for i, (set_indice, y) in tqdm(enumerate(zip(set_indices, labels)), total=len(set_indices)):
            data = get_data_sample(G, set_indice, y, hop_num, max_sp, cached_dir, i, debug)
            data_list.append(data)
    else:
        raise NotImplementedError('not support parallel processing now')
    return data_list

def load_cached_datalist(cached_dir, logger):
    ...
    data_list = filelist
    if logger is not None:
        logger.info("cached dataset {} loaded.".format(cached_dir.absolute()))
    return data_list
...
def get_data_sample(G, set_indice, y, hop_num, max_sp, root_dir, findex, debug=False):
...
def get_data(G, args, logger):
    ...
    train_loader, val_loader, test_loader = load_dataset(train_set, val_set, test_set, args.batch_size)
...
def load_dataset(train_set, val_set, test_set, batch_size):
    num_workers = 1
    train_loader = DataLoader(train_set, batch_size=batch_size, shuffle=True, pin_memory=True, num_workers=num_workers, collate_fn=Collater(follow_batch=[]))
    val_loader = DataLoader(val_set, batch_size=batch_size, shuffle=True, pin_memory=True, num_workers=num_workers, collate_fn=Collater(follow_batch=[]))
    test_loader = DataLoader(test_set, batch_size=batch_size, shuffle=True, pin_memory=True, num_workers=num_workers,   collate_fn=Collater(follow_batch=[]))
